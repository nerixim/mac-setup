...
- **A vendor's payload, schema or feature semantics come from the vendor's docs, not from memory.** Hand-building a provider's event, request or config from recall looks right and fails silently — the consumer accepts it and does nothing, so there is no error to chase. Read the reference page and copy the documented shape before the first attempt, not after the second failure. Most vendors publish machine-readable docs for this: AWS serves `llms.txt` indexes and a `.md` twin for every docs page (swap `.html` for `.md`), and others do the same. One fetch beats two failed round trips against live infrastructure.
